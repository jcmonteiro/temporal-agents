# Plans — roadmap and cross-plan decision record

Each feature has its own `brief.md` (why/what), `implementation-brief.md`
(constraints), and `slices/` (task-oriented, demoable units). This file holds
what no single feature owns: the order the features ship in, why that order is
forced, and the decisions that several features depend on.

## Feature order

| # | Plan | Outcome | Depends on |
|---|------|---------|-----------|
| 1 | [`locations`](./locations/) | every item reports where it runs; the overview groups work into planets per location, collapsing toward parents | — |
| 2 | [`authentication`](./authentication/) | the hub authenticates through an OIDC provider, SSO-ready, with server-held tokens | 1 (nothing hard, ships beside it) |
| 3 | [`prompts`](./prompts/) — foundation slices | prompts are stored, scoped per location, versioned, and every run records which prompt version it used | 1 |
| 4 | [`launching-work`](./launching-work/) | an operator starts and re-runs agent work from a dedicated page | 1, 2 |
| 5 | [`steering`](./steering/) | a review round waits for the operator, who guides it by text or by being grilled, then builds | 1, 2, 3 (foundation) |
| 6 | [`prompts`](./prompts/) — configuration slices | prompts are editable globally and per location, with resets to inherited and to factory | 3 |
