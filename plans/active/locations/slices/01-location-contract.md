...
- [x] Add the location model to the application core as a **closed set of
      ...
      matching variant.
- [x] Validate on construction: a directory is absolute and cleaned; a ref is
      non-empty and bounded; the unknown variant takes no parent.
- [x] Add registry construction: given the locations referenced by a response,
      emit the **ancestry closure**, ordered parents-first, ids stable and
      deterministic, labels server-computed.
- [x] Extend the wire representations with `locationId` on fleets, runs,
...
- [x] Extend the published schema with the union (discriminated on the variant)
      and the registry property; extend the schema tests rather than rewriting
      them.
- [x] Assert **deterministic serialisation** of the registry, and that entity tags
      stay stable for an unchanged read (IB §1).
- [x] Unit tests (behaviour, no database): a response referencing several
...
- [x] Contract tests: existing consumers decode the new payload unchanged; no
