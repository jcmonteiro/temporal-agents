...
| [`launching-work`](./launching-work/) | the hub has real routes and dedicated pages; an operator registers a place, starts a develop or review pass in it, lands on the run, and repeats a past run | fully delivered; the fleet page it routes to stays a marked placeholder; instruction settings were delivered by `prompts` |
| [`prompts`](./prompts/) | prompts are stored, scoped, versioned, attributed, editable globally and per place, and resettable to inherited or shipped values | fully delivered |
