...
// openPublishedConfiguration connects the worker to the scoped configuration store
// and publishes the values this build ships into it, so an upgrade that improves one
// reaches every place that has not overridden it.
//
// Both halves are fail-fast, and for the same reason the execution store is: these
// values are what the agent is told and what the tool does, so a worker that cannot
// reach the store — or cannot publish what it ships — must not take work and resolve
// to something nobody chose. Publication applies no DDL (the schema is the explicit migrate
...
func openPublishedConfiguration(ctx context.Context) *scopedpg.Store {
	dsn, err := databaseURL()
	if err != nil {
		fatalf("%v", err)
	}
	...
	defer cancel()
	store, err := scopedpg.Open(connectCtx, dsn)
	if err != nil {
		fatalf("could not reach the configuration store: %v", err)
	}
