...
// The adapter is tested against a real Postgres, because everything it exists for
// is the database's behaviour and not Go's: the deleting read that makes a callback
// single-use even under a race, the foreign key that keeps a session from naming an
// identity nobody recorded, the upsert that refreshes a display name, and the
// migration itself. A mock would exercise none of it.
//
...
// container, with the identity schema applied, and closes it afterwards.
