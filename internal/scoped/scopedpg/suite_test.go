...
// the database's behaviour and not Go's: the per-key lock that lets two starting
// processes publish one version between them, the foreign key that stops a pointer
// naming a version nobody stored, the insert-only version table, and the migration
// itself. A mock would exercise none of it.
//
