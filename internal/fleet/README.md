...
- **`FleetPlanWorkflow`** — the *plan* step. A read-only Pi agent decomposes a
  high-level goal into a validated dependency graph (`FleetPlan`), stores it
  under its handle, and returns it for the user to review before executing. It
...
`activities.go` / `ports.go`. Both workflows also record themselves durably
through the `execstore` port (`recording.go`): planning as a `fleet-plan`
execution and the orchestration as a `fleet` execution whose per-node breakdown
is the only home for a **skipped** node (a skipped node starts no child workflow,
so it has no run of its own to record).
...
```mermaid
flowchart TD
    start([FleetPlanInput: Goal, WorkDir, PlanID, Name]) --> rec["PersistFleetPlanWorkflowState<br/>(status: running)"]
    rec --> gp
    ...
    result --> store["StorePlan activity<br/>(upsert the plan under PlanID)"]
    store --> notify["wfnotify: 'Fleet plan ready'<br/>(node count + handle + planning tokens)"]
    ...
    done -.->|defer| endrec["PersistFleetPlanWorkflowState<br/>(terminal: status, tokens, plan handle)"]
    fail -.->|defer| endrec
```

**Notes**

- The plan store is the plan's **sole** source of truth (there is no plan file),
  so `StorePlan` is authoritative: a plan that cannot be written fails planning
  rather than announcing a handle that resolves to nothing. The handle is
  generated by the caller and passed in, so a retried write stores the same plan
  under the same handle.
- Recording is likewise authoritative (idempotent upsert on the Temporal run ID):
  a run that cannot record its start does not begin. The terminal write runs on a
  disconnected context so a cancelled run still settles its record, and if it
  fails anyway the run keeps its outcome and the failure is reported (see
  `wfrecord.TerminalWriteFailed`) rather than discarding every node's work.
