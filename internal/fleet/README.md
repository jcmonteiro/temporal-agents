...
- Recording is likewise a hard dependency (idempotent upsert on the Temporal run
  ID, must succeed), and the terminal write runs on a disconnected context so a
  cancelled run still settles its record.
